...
**Cast sheets:** every member reads their instructions from their own bundled markdown file — `ios/Vesper/Vesper/Sheets/{vesper,mika,chat}.sheet.md` — so switching cast really switches the sheet, not just the skin. Vesper's sheet is the same text as `companion/personality.py`'s `BASE_PERSONA`; her emotion pipeline layers the live state block on top of it (Away builds that prompt on the phone; at home the Mac builds it server-side, exactly as before). Mika's and Chat's sheets ride whole: used directly as the system prompt on the Away leg, and sent as `persona_override` to the Mac's `phone_api` on the home leg. `project.pbxproj` pins all three sheets into the target's **Copy Bundle Resources** via a build-phase membership exception (current Xcode often copies loose `.md` files from a buildable folder on its own, but the explicit pin makes it guaranteed rather than version-dependent); `bundledSheet` tries both name splits (`name.sheet` + `md` and `name` + `sheet.md`) in both the bundle root (where the copy lands them) and a `Sheets/` subfolder, and each sheet's full text is embedded as the fallback — so even a botched bundle copy can't flatten Mika into a one-line blurb. Edit a sheet → rebuild → that member changes; if a Mac keeps richer canonical sheets (e.g. `~/Grok-Bot/.vesper/cast/mika.sheet.md`), copy them over the bundled files (`cp ~/Grok-Bot/.vesper/cast/mika.sheet.md ios/Vesper/Vesper/Sheets/mika.sheet.md`) and rebuild so phone Mika matches Mac Mika. Quick check after a fresh build: switch to Mika, ask "who are you and who am I?" — she should answer as Tigger's dry-funny partner in her `eve` voice, not as Vesper; Chat should answer with no persona at all.
